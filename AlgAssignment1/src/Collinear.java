...
class Collinear
{
   ...
   // ----------------------------------------------------------
     ...
     *
     *  Order of growth: N1*N2*N3(N1 - running time for a1,N2 - running time for a2,N3 - running time for a3)
     *
     *  Explanation: Triple for-loop
     */
    static int countCollinear(int[] a1, int[] a2, int[] a3)
    {
      ...
      for(int i = 0; i < a1.length; i++)
      {
    	  for(int j = 0; j < a2.length; j++)
    	  {
    		  ...
    		  {
    			  if((a1[i]*(-1) + a2[j]*(2) + a3[k]*(-1)) == 0) //x1*(y2−y3) + x2*(y3−y1) + x3*(y1−y2)=0
    			  {
    				  count++;
    			  }
    		  }
     ...
     *  You should adequately explain your answer. Answers without adequate explanation will not be counted.
     *
     *  Order of Growth: N1*N2*logN3
     *
     *  Explanation: binarySearch(explained below) within a double loop.
     *
     *
     */
    static int countCollinearFast(int[] a1, int[] a2, int[] a3)
    {
    	...
    	for(int i = 0; i < a1.length; i++)
        {
      	  for(int j = 0; j < a2.length; j++)
      	  {
      		 if(binarySearch(a3,-(((a1[i]*(-1) + a2[j]*(2))/-1))))
     ...
     *
     *  Explanation: Every time the loop is executed the array is divided in half
     *
     */
